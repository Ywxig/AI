...
def GetTokenInMessage(file : str, message : list, procent = 0.65) -> list:
    ...
    for q in textdata:
        ...
        for i in q.split():
            try:
                res = WordOperations.is_word(message[count], i.split()[count])# res = результат на сколько слово из сообщений похоже на слово в ключе
                if res > procent:
...
def ChatCraft(message : list, file_dataset : str, iterabel_counts = 64):
    ...
    matrix = WordOperations.Text_to_matrix(". ".join(iner_data), 2)
    chain_matrix_material = {}
    for i in str_:
        Freq = WordOperations.Frequency_matrix(matrix, i, procent=0.75)
        chain_matrix_material[i] = Freq
    keywords = WordOperations.Keywords(". ".join(iner_data))
    if keywords == []:
    ...
    for i in range(iterabel_counts):
        ...
        if last_item_text != "":
            try:
                if rand2 > 0 and i < 3:
                    arr = WordOperations.Find_matrix_row(matrix, last_item_text, procent=0.65)
